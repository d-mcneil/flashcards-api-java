...
// ############################################################################################################
// User getUserByUsername(int username)
// ############################################################################################################

    @Test
    public void getUserByUsername_returns_correct_user_information_for_user_that_is_active() {
        User user2 = sut.getUserByUsername(TEST_USER_2.getUsername());
        assertUsersMatch(TEST_USER_2, user2);
    }

    @Test
    public void getUserByUsername_returns_null_for_user_that_is_not_active() {
        User user1 = sut.getUserByUsername(TEST_USER_1.getUsername());
        Assert.assertNull(user1);
    }

    @Test
    public void getUserByUsername_returns_null_for_user_that_does_not_exist() {
        User nullUser = sut.getUserByUsername("thisuserdoesntexist");
        Assert.assertNull(nullUser);
    }

    @Test
    public void getUserByUsername_returns_null_for_empty_string_username() {
        User nullUser = sut.getUserByUsername("");
        Assert.assertNull(nullUser);
    }

    @Test
    public void getUserByUsername_returns_null_for_null_username() {
        User nullUser = sut.getUserByUsername(null);
        Assert.assertNull(nullUser);
    }
...
// #################################################################
    ...
    public void getHashedPasswordByUsername_returns_correct_hash_for_user_that_is_active() {
        String hash2 = sut.getHashedPasswordByUsername(TEST_USER_2.getUsername());
        Assert.assertEquals("2".repeat(60), hash2);
    }

    @Test
    public void getHashedPasswordByUsername_returns_null_for_user_that_is_not_active() {
        String nullHash = sut.getHashedPasswordByUsername(TEST_USER_1.getUsername());
...
// #################################################################
    ...
    public void createUser_returns_correct_user_information_and_correctly_sets_hashed_password_and_user_has_correct_information_when_retrieved() {
        ...
        if (createdUserTimestamp.getYear() != rightNow.getYear() ||
            ...
            Assert.fail("Timestamp generated by database does not match timestamp generated by server. Try again and it might.");
        }
...
// ****************************************************************************************************************************************************************
    ...
    private void assertUsersMatch(User expected, User actual) {
        ...
        Assert.assertEquals(expected.getUsername().toLowerCase(), actual.getUsername().toLowerCase());
